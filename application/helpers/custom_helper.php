    ...
    function import_csv($data, $table, $static_columns = [])
    {
        ...
        if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)) {
            if (is_uploaded_file($_FILES['file']['tmp_name'])) {
                ...
                if ($header === false) {
                    echo "No header found from csv file";
                    return false;
                }
                ...
                while (($row = fgetcsv($csvFile)) !== FALSE) {
                    ...
                    foreach ($column_mapping as $db_column => $csv_index) {
                        if (!empty($row[$csv_index])) {
                            $row_data[$db_column] = $row[$csv_index];
                        }
                    }
                ...
                if (empty($csv_data)) {
                    echo "No valid columns found from your file";
...
if (!function_exists('get_reservations_by_user_id')) {
    function get_reservations_by_shop_id($user_id)
    {
        // Get CodeIgniter instance
        $ci = &get_instance();
        $ci->load->database(); // Load the database

        // Query to get reservations by shop_id
        $ci->db->where('user_id', $user_id);
        $query = $ci->db->get('reservation_date'); // 'reservation' is the name of your table
