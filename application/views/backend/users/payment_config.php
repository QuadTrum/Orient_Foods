	...
	<?php if (check() == 1 && $this->security->online_payment() == 1) : ?>
		<div class="col-md-7">
			...
			<form class="email_setting_form mt-50" action="<?= base_url('admin/restaurant/add_payment_method'); ?>" method="post" enctype="multipart/form-data" autocomplete="off">
				...
				<?php if (shop_active('iyzico_status') == 1) : ?>
				...
				<?php if (shop_active('offline_status') == 1) : ?>
					<div class="card moyasar">
						<div class="card-header mt-1">
							<div class="paymentImg">
								<img src="<?php echo base_url('assets/frontend/images/payout/offline.svg'); ?>" alt="pMethod">
							</div>
						</div>
						<div class="card-body">
							<div class="row">
								<?php $offline = isJson($settings['offline_config']) ? json_decode($settings['offline_config']) : ''; ?>
								<div class="form-group  col-md-6 col-sm-6 col-xs-12">
									<label for=""><?= !empty(lang('status')) ? lang('status') : "Status"; ?></label>
									<div class="">
							...
							<div class="row">
								<div class="col-md-12 col-sm-12 col-xs-12">
									<div class="form-group">
										...
										<div class="">
											<textarea name="offline_details" id="offline_details" class="form-control textarea" cols="5" rows="5" placeholder="<?= lang('bank_details'); ?>"><?= !empty($offline->offline_details) ? html_escape($offline->offline_details) : '';  ?></textarea>
										</div>
										<div class="row mt-20">
											<div class="col-md-6">
												<label class="custom-checkbox"><input type="checkbox" name="is_transaction_field" id="" <?= isset($offline->is_transaction_field) && $offline->is_transaction_field == 1 ? "checked" : ""; ?>><?= lang('enable_transaction_id_field'); ?></label>
											</div>

											<div class="col-md-6">
												<label class="custom-checkbox"><input type="checkbox" name="is_document_field" id="" <?= isset($offline->is_document_field) && $offline->is_document_field == 1 ? "checked" : ""; ?>><?= lang('enable_payment_documemt_uploader'); ?></label>
											</div>
										</div>
									</div>
								</div>
