...
class Common_m extends CI_Model
{
	...
	public function get_my_categories($user_id, $language = null, $limit = 0)
	{
		...
		foreach ($query as $key => $value) {
			...
			$this->db->where('s.language', !empty($language) ? $language : $this->site_lang);
	...
	// sub category
	public function sub_category_details($user_id, $sub_cat_id, $is_md5 = false)
	{
		$id = md5('1');
		...
		$this->db->where('s.status', 1);
		$this->db->where('s.language', $this->language);
		$this->db->order_by('s.orders', 'ASC');
		$query = $this->db->get();
		$query = $query->row();
		return $query;
	...
	public function get_item_by_sub_cat_id($user_id = 0, $cat_id = 0, $sub_cat_id = 0, $limit = 0, $per_page = 0, $offset = 0, $total = 0)
	{
		...
		$this->db->where('c.language', $this->language);
		if (isset($_GET['item']) && !empty($_GET['item'])) {
	...
	public function get_offers($shop_id)
	{
		...
		foreach ($query as $key => $row) {
			...
			$this->db->from('items i');
			$this->db->where_in('i.id', json_decode($row['item_ids']));
			$query2 = $this->db->get();
			$query2 = $query2->result_array();
			$query[$key]['items'] = $query2;
		}
